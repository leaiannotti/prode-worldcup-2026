...
describe('formatPrizeChangedEvent', () => {
  it('formats a regular member change', () => {
    const event = makeEvent({ rank: 1, previous_value: 'Pizza', new_value: 'Asado', actor_is_admin: false, actor_name: 'Juan' })
  ...
  it('formats an admin change with marker', () => {
    const event = makeEvent({ rank: 2, previous_value: 'Cerveza', new_value: 'Vino', actor_is_admin: true, actor_name: 'Lea' })
  ...
  it('falls back to "someone" when actor_name is missing', () => {
    const event = makeEvent({ rank: 3, previous_value: 'Medalla', new_value: 'Nada', actor_is_admin: false })
